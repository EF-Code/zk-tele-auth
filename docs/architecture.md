...
4. TON verifier to launchpad/economics: `PrivaPurchaseLaunchpad` composes the verifier core with immutable launch policy, atomic action/query/identity/inventory accounting, sender-bound recipient checks, native-TON value accounting, storage reserve, and refundable overpayment credits. It deliberately emits no asynchronous settlement action; any jetton/NFT settlement or credit-withdrawal adapter needs a separate review.
