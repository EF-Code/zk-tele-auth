...
if (manifest.status !== 'development-only') throw new Error('development verification refuses a production-labelled manifest');
for (const [circuit, entry] of Object.entries(manifest.circuits || {})) {
  if (entry.status !== 'development-only') throw new Error(`${circuit} is not development-only`);
...
console.log(`✓ development artifact hashes and exported verification keys verified (${Object.keys(manifest.circuits || {}).length} circuits)`);
process.exit(0);
