...
const temporaryDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zk-tele-auth-ton-check-'));
const generatedPath = path.join(temporaryDir, 'generated.tolk');

function extractVerifierConstants(source) {
  const match = source.match(/fun ZkTeleAuthVerifier\.create\(\): ZkTeleAuthVerifier \{[\s\S]*?\n\}/);
  if (!match) throw new Error('unable to locate ZkTeleAuthVerifier.create() constants');
...
try {
  ...
  const compilation = await runTolkCompiler({
    entrypointFileName: path.relative(root, contractPath),
  ...
  console.log(`  ✓ TON verifier key matches artifacts and Tolk compiles (${compilation.codeHashHex})`);
} finally {
